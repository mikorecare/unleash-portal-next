...
const DefaultDashboardByThreesContainer = ({
    dashboardData,
    size = "sm",
    cardHeightInPx = "152",
}: {
    ...
    return (
        <div className={`w-full mb-4 ${sizesMap.get(size)}`}>
            <div className="grid grid-cols-[repeat(auto-fit,_minmax(390px,_1fr))] gap-4">
                {renderMetricCards(dashboardData).map((card, index) => (
                    <DashboardDefaultCard
                        key={index}
                        data={card}
                        minHeight={cardHeightInPx}
                    />
                ))}
            </div>
        </div>
...
export default DefaultDashboardByThreesContainer;
