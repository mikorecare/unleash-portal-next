...
const ErrorHandler = () => {
  ...
  const { signInErrorStatus, title, subtitle }: ErrorHandlerProps = useSelector(
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 font-montserrat">
        ...
        <p className="font-medium text-xs text-website-gray">{subtitle}</p>
      </div>
      <IoCloseOutline className="w-10 h-10 text-website-gray p-2" onClick={closeError} />
    </div>
  );
};
