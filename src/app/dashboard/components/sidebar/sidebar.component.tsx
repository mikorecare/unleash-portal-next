...
const DashBoardSidebar = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
        <div className="flex h-[100vh] bg-website-color justify-center items-center">
            <div
                className={`transition-all duration-300 z-10 bg-website-light h-[95.75%] ml-5 shadow-sm flex flex-col pb-4 rounded-lg`}
            >
                {/* Dashboard Header */}
                    ...
                    <div className={`flex flex-row gap-1 flex-1`}>
                        ...
                        >
                            <p className="text-[10px] font-montserrat text-white font-medium">
                                {UserRoleFactory.ROLES_CONFIGS[role]?.tag.name}
                            </p>
                        </div>
                    </div>
                    <div
                        className={`flex justify-center items-center cursor-pointer ${
                            open ? "h-full w-7" : "hidden"
                        }`}
                ...
                <div className={`flex flex-col items-center mt-8 flex-1`}>
                    {SIDEBAR_ITEMS.filter((item) =>
                        item.roles.includes(role as UserRoles)
                    ).map((item, index) => (
                        <SidebarItem
                            key={index}
                ...
                <div
                    className={`flex flex-row transition-all duration-200 font-montserrat mb-4 text-sm font-medium select-none text-website-gray-400 rounded-lg cursor-pointer items-center bg-website-light hover:brightness-95
                            ${
                                open
                                    ? "opacity-[.68] mx-3 p-5 "
                                    : "opacity-100 mx-auto p-3"
                            }`}
                    onClick={logout}
                >
                    <PiSignOutBold
                        className={`${open ? "w-5 h-5" : "w-7 h-7"}`}
                    />
                    <p
                        className={`text-sm transition-all duration-100 ease-in-out ${
                            open
                                ? "w-full ms-4"
                                : "w-0 text-nowrap overflow-hidden"
                        }`}
                    >
                        Logout
                    </p>
            ...
            <div
                className={`flex-1 h-[95.75%] px-6 overflow-y-scroll duration-150 ease-in-out`}
            >
                {role && <Header
                    title={UserRoleFactory.ROLES_CONFIGS[role].header}
